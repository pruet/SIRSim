...
class SirSimulatorGUI:
    ...
    def build_tab_execution(self):
        ...
        # Max Steps (t)
        ttk.Label(frame_mc, text="Max Ticks (Simulation steps):", background="#ffffff").grid(row=0, column=0, anchor=tk.W, pady=6)
        self.spin_steps = ttk.Spinbox(frame_mc, from_=10, to=2000, width=8)
        self.spin_steps.set(500)
        self.spin_steps.grid(row=0, column=1, anchor=tk.W, padx=10)
        
        # MC Runs (n)
        ttk.Label(frame_mc, text="Monte Carlo independent runs:", background="#ffffff").grid(row=1, column=0, anchor=tk.W, pady=6)
        self.spin_runs = ttk.Spinbox(frame_mc, from_=1, to=500, width=8)
        self.spin_runs.set(50)
        self.spin_runs.grid(row=1, column=1, anchor=tk.W, padx=10)
        
        # Alignment (a)
        ttk.Label(frame_mc, text="Time Alignment Averaging:", background="#ffffff").grid(row=2, column=0, anchor=tk.W, pady=6)
