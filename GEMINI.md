...
- **suppression_strategies.py**: A modular strategy module housing the epidemic suppression registry (`@register_strategy`). Contains:
  ...
  6. `louvain_edge_suppression`: Detects communities using NetworkX's weighted Louvain algorithm, then suppresses the top highest-probability inter-community bridging edges, achieving high containment performance.
